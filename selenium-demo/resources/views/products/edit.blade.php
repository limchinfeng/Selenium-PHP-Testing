...
<div class="flex flex-col gap-5 max-w-[1440px] mx-auto p-5 mt-20 border-2 border-white">
    ...
    <form method="POST" action="{{ route('products.update', $product->id) }}" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="name" id="name"  class="border border-gray-300 rounded-md px-3 py-2 w-full" value="{{ old('name', $product->name) }}" required>
        </div>

        <div>
            <label for="description" class="block text-white">Description:</label>
            <textarea name="description" id="description" required class="border border-gray-300 rounded-md px-3 py-2 w-full" >{{ old('description', $product->description) }}</textarea>
        </div>
